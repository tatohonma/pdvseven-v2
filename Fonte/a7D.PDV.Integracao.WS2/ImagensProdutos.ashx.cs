...
namespace a7D.PDV.Integracao.WS
{
    ...
    public class ImagensProdutos : System.Web.UI.Page, IHttpHandler
    {
        ...
        public static double TamanhoThumbAltura
        {
            get
            {
        ...
        public static double TamanhoThumbLargura
        {
            get
            {
        ...
        public override void ProcessRequest(HttpContext context)
        {
            ...
            //}
            string id = Page.RouteData.Values["id"] == null ? string.Empty : Convert.ToString(Page.RouteData.Values["id"]);
            ...
            if (string.IsNullOrWhiteSpace(id) == false)
            {
                ...
                lock (cachefile)
                {
                    if (RetornarDoCache(context.Response, id, cachefile, pi.Imagem.Extensao, thumb))
                        return;
                }
                ...
                if (pi != null && pi.Imagem != null && pi.Imagem.IDImagem.HasValue)
                {
                    ...
                    using (var imgDeDados = new ConversorByteArrayParaImagem(pi.Imagem.Dados).Imagem)
                    {
                        if (thumb)
                        {
                            var image = CreateThumb(pi.Imagem.Dados);
        ...
        private bool RetornarDoCache(HttpResponse response, string id, string cachefile, string extensao, bool thumb)
        {
        ...
        public static DadosImagem CreateThumb(byte[] imagem)
        {
            int newWidth = (int)TamanhoThumbLargura;
            int newHeigth = (int)TamanhoThumbAltura;
