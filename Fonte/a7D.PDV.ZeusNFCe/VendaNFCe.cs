...
namespace a7D.PDV.ZeusNFCe
{
    public class VendaNFCe
    {
        ...
        public string ObterNfeValidadaEnvia(PedidoInformation pedido)
        {
            var nfe = ObterNfeValidada(pedido.IDPedido.Value, 1);

            var servicoNFe = new ServicosNFe(ConfiguracaoServico.Instancia);
            var retornoEnvio = servicoNFe.NFeAutorizacao(Convert.ToInt32(2), IndicadorSincronizacao.Sincrono, new List<NFe.Classes.NFe> { nfe }, false);
