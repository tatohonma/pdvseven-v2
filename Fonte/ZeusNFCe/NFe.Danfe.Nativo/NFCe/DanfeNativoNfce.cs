...
namespace NFe.Danfe.Nativo.NFCe
{
    public class DanfeNativoNfce
    {
        ...
        public void GerarPNG(int width = 280, string filename = null, Stream sw = null)
        {

            // Feito esse de cima para poder pegar o tamanho real da mesma desenhando
            using (Bitmap bmp = new Bitmap(width, 70000))
            {
                ...
                using (Bitmap bmpFinal = new Bitmap(width, _y))
                {
                    using (Graphics g = Graphics.FromImage(bmpFinal))
                    {
                        ...
                        g.DrawImage(bmp, 0, 0);
                        if (filename != null)
                            bmpFinal.Save(filename, ImageFormat.Png);
                        else if (sw != null)
                            bmpFinal.Save(sw, ImageFormat.Png);
                    }
                }
            }
        }

        private void GerarNfCe(Graphics graphics, int larguraLinha = 284)
        {
            var g = graphics;

            var larguraLogo = 64;
            int larguraLinhaMargemDireita = larguraLinha - 7; //277;
            ...
            foreach (det detalhe in det)
            {
                ...
                AdicionarTexto unidadeSigla = new AdicionarTexto(g, detalhe.prod.uCom.Length > 2 ? detalhe.prod.uCom.Substring(0, 2) : detalhe.prod.uCom, 7);
